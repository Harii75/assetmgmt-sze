    ...
    const fetchData = async () => {
      try {
        ...
        const parseDate = (dateStr) => {
          ...
          dateStr = dateStr.replace(/"/g, "").trim(); // Remove extra quotes
          const parts = dateStr.split("-"); // Split YYYY-MM-DD
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gray-100">
              ...
              <th className="p-2 text-left">Beszerzés Dátuma</th>
            </tr>
          </thead>
          <tbody>
            {data.map((item, index) => (
              <tr key={index} className="border-t">
                ...
                <td className="p-2">{item.custom_fields?.Brutto_ertek?.value || "N/A"} HUF</td>
                <td className="p-2">
